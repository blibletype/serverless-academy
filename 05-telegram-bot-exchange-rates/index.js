...
bot.onText(new RegExp(buttons.menu.currency), async (ctx) => {
  await showCurrencyButtons(bot, ctx);
});

bot.onText(buttons.getAllCurrencyButtonsRegex(), async (ctx) => {
  await handleExchangeRates(bot, ctx);
});
